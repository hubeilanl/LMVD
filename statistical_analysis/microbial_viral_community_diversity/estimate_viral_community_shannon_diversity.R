...
output_file <- "viral_community_shannon_diversity_by_sample.tsv"
chunk_lines <- 500
...
if (length(header) != 1) {
  stop("The viral abundance table is empty: ", input_file)
}
...
results <- data.frame(
  SampleID = sample_ids,
  ViralShannon = shannon,
  ViralTotalAbundance = abundance_sum,
